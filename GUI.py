...
from interattack import double_encrypt, double_decrypt, middle_crack
from CBC import CBC
def int_(input):
    try:
        # 先尝试将字符串解释为十进制
    ...
    if ev0 == "拓展功能" and not win5_active:  # 窗口5 拓展功能
        ...
        while True:
            ...
            # 在下方if输入函数
                ...
                ciphertext = ascii((vals5[0]), int_(vals5[1]))
                sg.popup_scrolled('密文是: ' + str(ciphertext))  # 弹出密文
            elif ev5 == '解密':
                #     vals5[0]是字符串内容 vals5[1]是密钥内容
                plaintext = ascii((vals5[0]), int_(vals5[1]), True)
                sg.popup_scrolled('明文是: ' + str(plaintext))  # 弹出明文
    ...
    if ev0 == "三重加密" and not win3_active:  # 窗口1 普通加密
        ...
        while True:
            ...
            # 在下方if输入函数
                ...
                ciphertext = ascii(double_encrypt(vals3[0], int_(vals3[1]), int_(vals3[2])), int_(vals3[3]))
                sg.popup_scrolled('密文是：' + str(ciphertext))  # 弹出密文
            elif ev3 == '解密':
                #     vals3[0]是字符串内容 vals3[1]是密钥内容
                plaintext = double_decrypt(ascii(vals3[0], int_(vals3[3]), True), int_(vals3[1]), int_(vals3[2]))
                sg.popup_scrolled('明文是：' + str(plaintext))  # 弹出明文
